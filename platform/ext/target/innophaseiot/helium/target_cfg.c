...
//    }
    ...
    ret = Driver_SRAM0_MPC.ConfigRegion(NS_DATA_START, NS_DATA_LIMIT,
                                        ARM_MPC_ATTR_NONSECURE);
#if defined(PSA_API_TEST_NS) && !defined(PSA_API_TEST_IPC)
    ret = Driver_SRAM0_MPC.ConfigRegion(DEV_APIS_TEST_NVMEM_REGION_START,
                                        DEV_APIS_TEST_NVMEM_REGION_LIMIT,
                                        ARM_MPC_ATTR_NONSECURE);
#endif
    if (ret != ARM_DRIVER_OK) {
...
fih_int fih_verify_mpc_cfg(void)
{
    ...
    Driver_SRAM0_MPC.GetRegionConfig(NS_DATA_START, NS_DATA_LIMIT, &attr);
    if (attr != ARM_MPC_ATTR_NONSECURE) {
        FIH_RET(fih_int_encode(ARM_DRIVER_ERROR));
    }

#if defined(PSA_API_TEST_NS) && !defined(PSA_API_TEST_IPC)
    Driver_SRAM0_MPC.GetRegionConfig(DEV_APIS_TEST_NVMEM_REGION_START,
                                     DEV_APIS_TEST_NVMEM_REGION_LIMIT,
                                     &attr);
    if (attr != ARM_MPC_ATTR_NONSECURE) {
        FIH_RET(fih_int_encode(ARM_DRIVER_ERROR));
